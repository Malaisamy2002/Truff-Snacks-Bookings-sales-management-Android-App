...
function ArrangeFrame({
  ...
  return (
    <div
      // Native HTML drag events are useful with a mouse but are not reliable
      // in Android WebView. The large arrow controls below are the touch-safe
      // arrangement path, so every layout change remains possible by finger.
      draggable
      onDragStart={(e) => {
        e.stopPropagation();
    ...
    >
      <div className="flex flex-wrap items-center gap-1 px-1 pb-1">
        ...
        <button
          ...
          disabled={!canMoveUp}
          onClick={() => onMove(-1)}
          className="grid size-11 shrink-0 place-items-center rounded-lg text-muted-foreground disabled:opacity-30 hover:bg-muted hover:text-foreground"
        >
