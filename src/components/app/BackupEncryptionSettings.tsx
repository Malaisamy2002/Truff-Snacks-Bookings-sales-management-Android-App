    ...
    <Card className="frost">
      ...
      <CardContent className="space-y-3">
        <p className="text-sm text-muted-foreground">
          ...
          device, so the ledger and receipt photos aren't readable by anyone who only has access
          to that chat. Use the same passphrase on every device that backs up or restores this
          ledger.
        </p>
        <div className="flex gap-2">
          <div className="relative flex-1">
            <Input
              ...
              placeholder={loaded && !saved ? "Set a backup passphrase" : "Backup passphrase"}
              className="pr-9"
            />
            <button
              type="button"
        ...
        {loaded && !saved && (
          <p className="text-xs text-destructive">
            No passphrase set yet — backups can't be sent until one is saved here.
          </p>
        )}
        <p className="text-xs text-muted-foreground">
          If this is lost, backups already sent can't be decrypted. Write it down somewhere safe.
        </p>
      </CardContent>
    </Card>
  );
