      ...
      <Card className="frost">
        <CardContent className="space-y-4 p-4">
          <p className="text-sm text-muted-foreground">
            ...
            it asks to archive the oldest year: sent to your configured Telegram chat, then
            {isDesktop() ? " saved" : " downloaded"} as “Turf bookings and sales - YEAR.db”
            {isDesktop() ? " wherever you choose" : " for your Documents folder"}, and removed only
            after both succeed. Telegram backup must be set up first (Settings → Telegram backup).
          </p>
        ...
        <AlertDialog open={confirmArchive} onOpenChange={setConfirmArchive}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This sends {pick || "the selected year"}'s bookings, sales and expenses to your
                configured Telegram chat, downloads them as a separate local file, and only then
                permanently deletes that year's records from this device. This can't be undone from
                here — make sure Telegram is set up first, and that both the upload and download
                succeed before relying on it.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
