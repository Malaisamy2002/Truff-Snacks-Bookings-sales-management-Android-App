...
export function BillingSettingsCard() {
  ...
  const [billStartDraft, setBillStartDraft] = useState(String(settings.billStartNo));
  ...
  useEffect(() => setGstRateDraft(String(settings.gstRate)), [settings.gstRate]);
  useEffect(() => setBillStartDraft(String(settings.billStartNo)), [settings.billStartNo]);
  useEffect(() => setBillPrefixDraft(settings.billPrefix), [settings.billPrefix]);
  useEffect(() => {
    setTaxRateDrafts((current) => {
      const next: Record<string, string> = {};
      for (const tax of settings.customTaxes) next[tax.id] = current[tax.id] ?? String(tax.rate);
      return next;
    });
  }, [settings.customTaxes]);

  const boundedNumber = (value: string, fallback: number, min = 0, max = 100) => {
                ...
                {settings.customTaxes.map((tax) => (
                  ...
                  >
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setTaxRateDrafts((current) => ({ ...current, [tax.id]: e.target.value }))
                      }
                      onBlur={() =>
                        set(
                          "customTaxes",
                          settings.customTaxes.map((t) =>
                            t.id === tax.id
                              ? { ...t, rate: boundedNumber(taxRateDrafts[tax.id] ?? "", 0) }
                              : t,
                          ),
                        )
                      }
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <div className="space-y-1">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setBillPrefixDraft(e.target.value.replace(/\s/g, "").slice(0, 10))
                  }
                  onBlur={() => {
              ...
              <div className="space-y-1">
                ...
                <Input
                  ...
                  onBlur={() =>
                    set("billStartNo", Math.floor(boundedNumber(billStartDraft, 1, 1, 9_999_999)))
                  }
                  placeholder="1"
                />
              </div>
