...
export function fullBackupSummary(backup: FullBackup): string {
  const rows = DATA_TABLES.reduce((n, t) => n + (backup.tables[t]?.length ?? 0), 0);
...
async function readReceiptBytes(path: string): Promise<Uint8Array> {
  if (isDesktop()) {
    if (!(await appDocumentExists(path))) throw new Error(`Missing on disk: ${path}`);
...
export async function buildFullBackup(deviceLabel = defaultDeviceLabel()): Promise<BuildFullBackupResult> {
  ...
  tables["receipt_hashes"] = (await db.receipt_hashes.toArray()) as unknown as Record<
    string,
    unknown
  >[];
...
export async function restoreFullBackup(
  ...
  for (const path of Object.keys(zip.files)) {
    ...
    if (!entry || entry.dir || path === MANIFEST_NAME || !path.startsWith("Receipts/")) continue;
...
function readMeta(): Meta {
  ...
  try {
    ...
    return raw ? { ...metaDefaults(), ...(JSON.parse(raw) as Partial<Meta>) } : metaDefaults();
  } catch {
    return metaDefaults();
  }
}
...
async function readExtraTokens(): Promise<string[]> {
  const raw = await readSecret(WEB_EXTRA_TOKENS_KEY, KEYRING_EXTRA_ACCOUNT, SECURE_STORE_EXTRA_KEY);
  if (!raw) return [];
  try {
...
async function writeExtraTokens(tokens: string[]): Promise<void> {
  ...
  return writeSecret(WEB_EXTRA_TOKENS_KEY, KEYRING_EXTRA_ACCOUNT, SECURE_STORE_EXTRA_KEY, raw);
}
...
export type PairingPayload = { botToken: string; chatId: string; extraBotTokens?: string[] };
...
export function decodePairingPayload(text: string): PairingPayload {
  ...
  return {
    ...
    extraBotTokens: Array.isArray(parsed.extraBotTokens) ? parsed.extraBotTokens.map(String) : [],
  };
}
...
export function chunkCount(totalBytes: number, chunkBytes = CHUNK_BYTES): number {
...
export function splitIntoChunks(bytes: Uint8Array, chunkBytes = CHUNK_BYTES): Uint8Array[] {
  ...
  for (let offset = 0; offset < bytes.length; offset += chunkBytes) {
    parts.push(bytes.subarray(offset, Math.min(offset + chunkBytes, bytes.length)));
  }
  return parts;
}
...
export function chunkFileName(session: string, part: number, total: number): string {
  return total === 1
...
export function parseChunkName(name: string): ParsedChunkName | null {
  const match = /^turf-ledger-full-backup-(.+?)\.zip(?:\.part(\d+)of(\d+))?$/.exec(name);
...
export type RemoteChunk = { fileName: string; fileId: string; messageId?: number };
export type ChunkGroup = { session: string; total: number; chunks: RemoteChunk[] };
 ...
 */
export function latestCompleteGroup(documents: RemoteChunk[]): ChunkGroup | null {
  ...
  for (const doc of documents) {
    ...
    if (!group.chunks.some((c) => c.fileName === doc.fileName)) group.chunks.push(doc);
  ...
  return {
    ...
    chunks: [...newest.chunks].sort(
      (a, b) => (parseChunkName(a.fileName)?.part ?? 0) - (parseChunkName(b.fileName)?.part ?? 0),
    ),
  };
}
...
export function retryAfterMs(body: unknown, attempt: number): number {
  const retryAfter = (body as { parameters?: { retry_after?: number } } | null)?.parameters
    ?.retry_after;
...
async function telegramFetch(url: string, init?: RequestInit): Promise<Response> {
  try {
    ...
    throw new Error("Couldn't reach Telegram — check the internet connection and try again.");
  }
}
...
async function uploadChunks(
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    for (let attempt = 1; ; attempt++) {
      ...
      if ((res.status === 429 || res.status >= 500) && attempt < MAX_CHUNK_ATTEMPTS) {
...
export async function uploadFullBackup(
  ...
  const deviceLabel = options.deviceLabel ?? cfg.deviceLabel ?? defaultDeviceLabel();
  ...
  rememberLastUpload({ session, total: parts, messageIds, at: new Date().toISOString() });
...
export type ParsedYearArchiveName = { year: number; session: string; part: number; total: number };

/** Pure — reads year/session/part/total back out of a year-archive filename. */
export function parseYearArchiveName(name: string): ParsedYearArchiveName | null {
...
export function readLastYearArchiveUpload(year: number): LastYearArchiveUpload | null {
  if (typeof window === "undefined") return null;
  try {
    const raw = window.localStorage.getItem(`${YEAR_ARCHIVE_LAST_UPLOAD_PREFIX}${year}`);
...
export async function uploadYearArchive(
  ...
  const { messageIds, parts } = await uploadChunks(
    ...
    (part, total) => yearArchiveCaption(year, session, part, total, deviceLabel),
    options.onProgress,
  );
...
export function chunksFromUpdates(updates: TelegramUpdate[], chatId: string): RemoteChunk[] {
...
export async function downloadChunk(token: string, fileId: string): Promise<Uint8Array> {
  const file = await callApi<{ file_path?: string }>(token, "getFile", { file_id: fileId });
  if (!file?.file_path) throw new Error("Telegram didn't return a download path for that part.");
  const res = await telegramFetch(`${API_ROOT}/file/bot${token}/${file.file_path}`);
  if (!res.ok) throw new Error(`Couldn't download a backup part (HTTP ${res.status}).`);
...
export async function fetchLatestFullBackupArchive(
  ...
  if (!isTelegramConfigured(cfg))
    throw new Error("Add the bot token and chat ID before restoring from Telegram.");
  ...
  if (!group) {
    ...
    for (let i = 0; i < last.messageIds.length; i++) {
      const forwarded = await callApi<TelegramUpdate["message"]>(
        ...
        "forwardMessage",
        { chat_id: cfg.chatId, from_chat_id: cfg.chatId, message_id: last.messageIds[i] },
      );
    ...
    if (!group) throw new Error("Couldn't read the last backup's parts back from Telegram.");
  }

  const parts: Uint8Array[] = [];
  for (let i = 0; i < group.chunks.length; i++) {
    onProgress?.({ part: i + 1, total: group.chunks.length });
    parts.push(await downloadChunk(botTokenForChunk(cfg, i), group.chunks[i]!.fileId));
  }
...
export function restoreSummary(result: RestoreFullBackupResult): string {
  ...
  if (result.filesSkippedExisting > 0)
    parts.push(`${result.filesSkippedExisting} photo(s) already on this device`);
  if (result.filesSkippedUnmatched > 0)
    parts.push(`${result.filesSkippedUnmatched} photo(s) didn't match an expense`);
  if (result.filesCorrupted.length > 0)
    parts.push(`${result.filesCorrupted.length} photo(s) failed a checksum and were not restored`);
  return parts.join(" · ");
}
