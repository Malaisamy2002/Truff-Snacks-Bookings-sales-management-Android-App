...
export function buildReceiptPdf(doc: ReceiptDoc, s: PrintSettings = readPrintSettings()): jsPDF {
  ...
  const rollHeaderDrawW =
    showRollHeader && s.rollHeader && rollHeaderNaturalH > MAX_ROLL_HEADER_HEIGHT_MM
  ...
  const renderBody = (pdf: jsPDF, pageHeightMm: number): number => {
    ...
    const itemQtys = [
      "QTY",
      ...doc.lines.map((line) => (line.qty === undefined ? "" : String(line.qty))),
    ];
    const amountHeader = wide ? "AMOUNT" : "AMT";
    const itemAmounts = [amountHeader, ...doc.lines.map((line) => moneyText(line.amount ?? 0))];
    ...
    const itemRow = (no: string, label: string, qty: string, amount: string, bold = false) => {
      ...
      if (compactThermalTable) {
        const compactQtyW = Math.max(6, contentW - pdf.getTextWidth(amountText) - tableGap);
    ...
    const sheetItemTable = (lines: ReceiptLine[]) => {
      ...
      rows.forEach(({ it, labelLines }, i) => {
        ...
        pdf.text(fitToWidth(qty, qtyColW), qtyX, firstBaseline, { align: "right" });
        pdf.text(
    ...
    if (wide) {
      ...
      doc.lines.forEach((it, i) => {
        itemRow(
          ...
          pmoney(it.amount ?? 0, s.currencySymbol),
        );
...
export async function downloadReceipt(
  ...
  if (isAndroid()) {
    ...
    if (result.saved) {
      ...
      toast.error("Couldn't save PDF", { description: result.error ?? `${doc.fileName}.pdf` });
    }
    return result.saved;
  }
...
export async function previewReceipt(
  ...
  if (isAndroid()) {
    ...
    if (result.saved) {
      toast.success("Preview opened", { description: "The receipt PDF was saved to Downloads." });
    } else {
...
export async function printReceipt(
  ...
  if (isAndroid()) {
    ...
    const saveResult = await saveExportFile(bytes, `${doc.fileName}.pdf`, "application/pdf", true);
    if (saveResult.saved) {
      ...
      toast.error("Couldn't print the receipt", {
        description: printResult.error ?? saveResult.error ?? `${doc.fileName}.pdf`,
      });
    }
    return;
  }
