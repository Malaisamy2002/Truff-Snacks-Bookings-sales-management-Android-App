/**
 * AES-256-GCM encryption for backup payloads — audit item 1.4.
 *
 ...
 * Telegram doesn't need to be able to read that archive for the
 * backup/restore flow to work, so it's encrypted client-side before it
 * leaves the device and decrypted client-side after it comes back. This
...
export async function encryptBackup(
  ...
  new DataView(header.buffer).setUint32(MAGIC.length + 1, PBKDF2_ITERATIONS, false);
...
export async function decryptBackup(
  ...
  if (!isEncryptedBackup(container))
    throw new Error("This file isn't an encrypted backup produced by this app.");
  if (!passphrase) throw new Error("Enter the backup passphrase to restore this file.");
