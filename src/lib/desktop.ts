...
export function describeSaveError(e: unknown): string {
  ...
  if (/space|quota/i.test(text)) return "Not enough free storage on the device.";
  return text;
}
...
export async function saveExportFile(
  ...
  try {
    ...
    if (!result?.bytesWritten)
      return { saved: false, error: "The file was created but nothing was written to it." };
 ...
 */
export async function printPdfFile(bytes: Uint8Array, filename: string): Promise<PrintResult> {
  try {
    const { invoke } = await import("@tauri-apps/api/core");
