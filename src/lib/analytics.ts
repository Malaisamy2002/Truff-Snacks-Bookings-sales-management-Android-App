 ...
 */
export const dataDateRange = (src: Sources): { earliest: string; latest: string } | null => {
...
export function taxReport(
  ...
  return keys.map((k) => {
    ...
    for (const x of src.sales) if (inMonth(x.sale_date) && isFinancialSale(x)) addLines(x.total, x);
...
export function turfOccupancy(
  ...
  const byHour = hourAgg.map((agg, h) => row(`hr-${h}`, `${String(h).padStart(2, "0")}:00`, agg));
  ...
  return {
    ...
    avgSlotHours:
      financial.length > 0 ? Math.round((bookedHours / financial.length) * 100) / 100 : 0,
    cancelled: {
