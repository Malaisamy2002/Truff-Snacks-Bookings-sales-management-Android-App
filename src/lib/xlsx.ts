...
export async function exportWorkbook(
  ...
  if (!buffer || buffer.byteLength === 0) {
    toast.error("Excel export failed", { description: "No data was produced for the file." });
    return false;
  }
  ...
  if (isAndroid()) {
    ...
    toast.error("Couldn't save Excel file", { description: result.error ?? name });
    return false;
  }
  ...
  if (isDesktop()) {
    try {
      ...
      toast.error("Couldn't save Excel file", { description: describeSaveError(e) });
      return false;
    }
